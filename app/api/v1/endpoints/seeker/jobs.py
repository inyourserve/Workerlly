...
@router.get("/jobs/available", response_model=List[JobRead], dependencies=[Depends(role_required("seeker"))])
...
async def bid_on_job(job_id: str, bid: JobBid, current_user: dict = Depends(get_current_user)):
    user_id = current_user["user_id"]
